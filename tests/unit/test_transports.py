...
class TestSendRequest:
    ...
    def test_send_request_not_ready_raises_error(self):
        """Test that non-READY status raises error."""
        transport = MockableL1Transport()
        ...
        assert "Chip status is not ready" in str(exc_info.value)
        assert hex(CHIP_STATUS_NOT_READY) in str(exc_info.value)

    def test_send_request_busy_raises_error(self):
        """Test that BUSY status raises error."""
        transport = MockableL1Transport()
